 ...
 * Information:
 * 
 * https://github.com/Overv/VulkanTutorial/blob/master/code/24_sampler.cpp
 * 
 * Extra information:
 * 
*/
...
void NeutronEngine::run() {
	initWindow();
	initVulkan();
	// Awake, Start functions for the Game
	initGame(); // Init the Game class, For game functions etc.
	game.Awake();
	game.Start();
	// Core functions
	mainLoop();
	// Clean up all the variables and memory that has been created.
	cleanup();
}
...
void NeutronEngine::initVulkan() {
	...
	setupDebugMessenger();
	createSurface();
	// Pick valid GPU and create devices.
	pickPhysicalDevice();
	createLogicalDevice();
	createSwapChain();
...
void NeutronEngine::createGraphicsPipeline() {
	...
	auto fragShaderCode = readFile(
		GetProjectPath"/source/Shaders/frag.spv");
...
void NeutronEngine::createDepthResources(){
...
VkFormat NeutronEngine::findSupportedFormat(const std::vector<VkFormat>& candidates, VkImageTiling tiling, VkFormatFeatureFlags features){
...
VkFormat NeutronEngine::findDepthFormat() {
...
bool NeutronEngine::hasStencilComponent(VkFormat format){
...
void NeutronEngine::createTextureImageView(){
...
void NeutronEngine::createTextureSampler(){
...
VkImageView NeutronEngine::createImageView(VkImage image, VkFormat format, VkImageAspectFlags aspectFlags){
...
void NeutronEngine::createImage(uint32_t width, uint32_t height, VkFormat format, VkImageTiling tiling, VkImageUsageFlags usage, VkMemoryPropertyFlags properties, VkImage& image, VkDeviceMemory& imageMemory) {
...
void NeutronEngine::transitionImageLayout(VkImage image, VkFormat format, VkImageLayout oldLayout, VkImageLayout newLayout){
...
void NeutronEngine::copyBufferToImage(VkBuffer buffer, VkImage image, uint32_t width, uint32_t height){
	VkCommandBuffer commandBuffer = beginSingleTimeCommands();
...
void NeutronEngine::createUniformBuffers() {
...
VkCommandBuffer NeutronEngine::beginSingleTimeCommands() {
...
void NeutronEngine::endSingleTimeCommands(VkCommandBuffer commandBuffer){
...
void NeutronEngine::updateUniformBuffer(uint32_t currentImage) {
